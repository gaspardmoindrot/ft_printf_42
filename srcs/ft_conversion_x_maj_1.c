...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_conversion_x_maj_1.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gmoindro <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/11 15:35:45 by gmoindro          #+#    #+#             */
/*   Updated: 2019/08/11 15:36:35 by gmoindro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_printf	ft_printm(unsigned long h, t_printf p_l)
{
	...
	if (p_l.prec == 0 && h == 0 && p_l.nb_t > 0)
		p_l.nb_t++;
	if (p_l.sign_neg > 0)
		p_l = ft_printm_1(h, p_l, len, i);
	else if (p_l.zero > 0 && p_l.prec < 0)
		p_l = ft_printm_2(h, p_l, len, i);
	else
	{
		if (p_l.ash > 0 && h != 0)
			i = i + 2;
		if (p_l.prec != 0 || h != 0)
			p_l.ret = p_l.ret + len;
		if (p_l.prec > len)
			p_l = ft_printm_3(h, p_l, len, i);
		else
			p_l = ft_printm_4(h, p_l, len, i);
	}
	return (p_l);
}
